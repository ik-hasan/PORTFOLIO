        ...
        <div className="max-w-3xl mx-auto">
          <div className="space-y-12">
            {workData.map((data, index) => (
              ...
              >
                {/* timeline */}
                <div className="absolute left-[-0.7rem] -top-0.5 w-6 h-6 rounded-full bg-purple"></div>

                {/* box */}
                <div className="bg-dark-300 rounded-2xl p-6">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-xl font-semibold">{data.role}</h3>
                    <span className="px-3 py-1 bg-purple/20 text-purple rounded-full text-xs">
                      {data.duration}
                    </span>
                  </div>

                  <h4 className="text-gray-400 mb-2">{data.company}</h4>
